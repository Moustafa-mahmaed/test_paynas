...
const AddRent = () => {
  ...
  const [luxIds, setLuxIds] = useState([]);
...
//       });
  ...
  const renderForm = ({ injectFormProps, handleSubmit ,setFieldValue ,errors  ,values  }) => (
...
console.log(JSON.stringify(formData));
    ...
    Axios
      .post(`${API_ENDPOINT}/api/UploadFile/FileUpload`,  formData  )
      .then(res => {
        console.log("*********1**********", res.data.filePaths[0])
             setFieldValue('offerAttachments[0]', res.data.filePaths[0])               
        setLoading(false)
      })
      .catch(error => {
        setLoading(false)
...
console.log(JSON.stringify(formData));
    ...
    Axios
      .post(`${API_ENDPOINT}/api/UploadFile/FileUpload`,  formData  )
      .then(res => {
        console.log("*********2**********", res.data.filePaths[0])
            //  setFieldValue('image2', res.data.filePaths[0])   
             setFieldValue('offerAttachments[1]', res.data.filePaths[0])               
...
console.log(JSON.stringify(formData));
    ...
    Axios
      .post(`${API_ENDPOINT}/api/UploadFile/FileUpload`,  formData  )
      .then(res => {
        console.log("*********3**********", res.data.filePaths[0])
            //  setFieldValue('image3', res.data.filePaths[0]) 
               
             setFieldValue('offerAttachments', [{...values.offerAttachments},res.data.filePaths[0]])               
...
console.log(JSON.stringify(formData));
    ...
    Axios
      .post(`${API_ENDPOINT}/api/UploadFile/FileUpload`,  formData  )
      .then(res => {
        console.log("*********4**********", res.data.filePaths[0])
                 console.log("values");  
                 
             setFieldValue('offerAttachments', [res.data.filePaths[0]])               
                 console.log(values['offerAttachments']);


            //  setFieldValue('image4', res.data.filePaths[0])               
        setLoading(false)
      })
      .catch(error => {
        setLoading(false)
...
<View stretch row  style={(typeId == 3 || typeId == 5 || typeId == 6  ) ?{ marginLeft:15 , marginRight:15 }:{   justifyContent: 'space-around' } } >
<View  width={40}  >
...
<Input  
   ...
   {(typeId == 1 || typeId == 2 || typeId == 4 ) ? 
      <View   width={40} >
      <Text style={styles.bedAndBathRoomsText}>{I18n.t('furnished')}</Text>
 ...
 {countryId !=="" ?
         <View stretch width={40} >
           <Picker
            ...
            //  for rent form
             setFieldValue('advertisementID', 2) 
         ...
         {typeId == 5 ? 
            ...
            apiRequest={{
              ...
              transformData: item => (
                ...
                {
                 label: rtl ? item.earthTypeNameAr : item.earthTypeNameEn,
                value: item.id,
              }
              ),
            }
...
<View stretch center style={{         }}>
  ...
  return (
      ...
      <Header  backgroundColor={colors.orange} title={I18n.t('AddSell')} 
